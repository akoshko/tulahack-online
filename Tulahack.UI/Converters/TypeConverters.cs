...
{
    private const string PC_StreamGeometryNotFound =
    ...
    public static FuncValueConverter<string, StreamGeometry> IconConverter { get; } =
        new(iconKey =>
        {
            if (iconKey is null)
            {
                return StreamGeometry.Parse(PC_StreamGeometryNotFound);
            }

            Application.Current!.TryFindResource(iconKey, out var resource);
            return resource as StreamGeometry ?? StreamGeometry.Parse(PC_StreamGeometryNotFound);
        });
}
