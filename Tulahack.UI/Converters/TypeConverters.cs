...
{
    private const string PcStreamGeometryNotFound =
    ...
    public static FuncValueConverter<string, StreamGeometry> IconConverter { get; } =
        new(iconKey =>
        {
            if (iconKey is null)
            {
                return StreamGeometry.Parse(PcStreamGeometryNotFound);
            }

            _ = Application.Current!.TryFindResource(iconKey, out var resource);
            return resource as StreamGeometry ?? StreamGeometry.Parse(PcStreamGeometryNotFound);
        });
}
